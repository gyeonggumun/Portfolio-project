  ...
  return (
    <>
      ...
      <div className="portfolio-container">
                  ...
                  <div className="detail-grid">
                    <div className="detail-text">
                      ...
                      <p>
                        ...
                        - <strong>악성 파일 방어 기제 구현:</strong> 파일명이나 확장자만으로 신뢰하지 단지 않고, 서버 측에서 MIME 검사와 Magic Number 교차 검증을 수행하도록 로직을 구현했습니다. PHP/JSP 등 실행 파일의 업로드를 차단하고, 업로드된 파일은 웹 루트 외부 비실행 경로에 격리 저장했습니다.<br/>
              ...
              {activeTab === 3 && (
                <>
                  ...
                  <div className="detail-grid">
                    <div className="detail-text">
                      ...
                      <p>
                        ...
                        <strong>Data/Planning:</strong> 공공데이터포털(농수산식품유통공사), Figma
                      </p>
                  ...
                  <div style={{ marginTop: '2rem', borderTop: '1px solid #e9ecef', paddingTop: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '2rem' }}>
                      <div className="gallery-item">
                        <img src="/p3-1.png" alt="ERD 구성도" />
                        <p>▲ ERD 구성도</p>
                      </div>
                      <div className="gallery-item">
                        <img src="/p3-2.png" alt="제품 서비스의 목표 시장 분석" />
                        <p>▲ 제품 서비스의 목표 시장 분석</p>
                      </div>
                      <div className="gallery-item">
                        <img src="/p3-3.png" alt="양조장 체험 선택페이지" />
                        <p>▲ 양조장 체험 선택페이지</p>
                      </div>
                      <div className="gallery-item">
                        <img src="/p3-4.png" alt="상품 구매 선택 페이지" />
                        <p>▲ 상품 구매 선택 페이지</p>
                      </div>
                      <div className="gallery-item">
                        <img src="/p3-5.png" alt="상품 구매 상세 페이지" />
                        <p>▲ 상품 구매 상세 페이지</p>
                      </div>
                      <div className="gallery-item">
                        <img src="/p3-6.png" alt="체험 관리 관리자 페이지" />
                        <p>▲ 체험 관리 관리자 페이지</p>
                      </div>
                    </div>
                  </div>
        ...
        ========================================= */}
        <div style={{ backgroundColor: '#f1f8ff', padding: '4rem 4vw', borderTop: '2px dashed #adb5bd' }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <h2 style={{ marginBottom: '1.5rem', color: '#1864ab', fontSize: '1.8rem', fontWeight: '700' }}>🔒 나만 보는 자리 (비공개 공간)</h2>
            <p style={{ marginBottom: '2rem', color: '#495057', fontSize: '1.05rem' }}>
            ...
            {!isLoggedIn ? (
              <div style={{ display: 'flex', gap: '1rem' }}>
                <button 
                  ...
                  style={{ padding: '1rem 2rem', cursor: 'pointer', backgroundColor: '#fff', border: '1px solid #ced4da', borderRadius: '8px', fontSize: '1.1rem', fontWeight: '600' }}>
                  🔑 새 패스키(기기) 등록
                </button>
                <button 
                  onClick={handleLogin} 
                  style={{ padding: '1rem 2rem', cursor: 'pointer', backgroundColor: '#228be6', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '1.1rem', fontWeight: '600' }}>
                  🔓 패스키로 로그인
                </button>
              </div>
            ) : (
              <div style={{ background: '#fff', padding: '3rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', border: '1px solid #dee2e6' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e9ecef', paddingBottom: '1.5rem', marginBottom: '2rem' }}>
                  <h3 style={{ margin: 0, fontSize: '1.5rem', color: '#212529' }}>나만의 데이터</h3>
                  <button onClick={handleLogout} style={{ padding: '0.6rem 1.5rem', cursor: 'pointer', borderRadius: '6px', backgroundColor: '#f8f9fa', border: '1px solid #dee2e6', fontWeight: '600' }}>로그아웃</button>
                </div>
                
                {/* 비공개 자료 3개 이상 출력 */}
                <ul style={{ lineHeight: '1.8', marginBottom: '3rem', listStyle: 'none', padding: 0 }}>
                  {privateData?.map(item => (
                    <li key={item.id} style={{ marginBottom: '1.5rem', padding: '1.5rem', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
                      <strong style={{ color: '#228be6', fontSize: '1.1rem', display: 'block', marginBottom: '0.5rem' }}>{item.title}</strong>
                      <span style={{ color: '#495057' }}>{item.content}</span>
                    </li>
                  ))}
                </ul>

                {/* 다중 패스키 관리 기능 */}
                <h3 style={{ borderBottom: '1px solid #e9ecef', paddingBottom: '1rem', marginBottom: '1.5rem', fontSize: '1.3rem', color: '#212529' }}>내 패스키 관리 (기기 목록)</h3>
                <ul style={{ listStyle: 'none', padding: 0 }}>
                  {passkeys.map(pk => (
                    <li key={pk.credentialID} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.2rem', background: '#f8f9fa', marginBottom: '0.8rem', borderRadius: '8px', border: '1px solid #e9ecef' }}>
                      <span style={{ fontSize: '1.05rem', color: '#343a40' }}>
                        <strong>{pk.name}</strong> <small style={{ color: '#868e96', marginLeft: '10px' }}>(등록일: {new Date(pk.createdAt).toLocaleDateString()})</small>
                      </span>
                      <button onClick={() => handleDeletePasskey(pk.credentialID)} style={{ background: '#fa5252', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' }}>
                        삭제
                      </button>
                    </li>
                  ))}
